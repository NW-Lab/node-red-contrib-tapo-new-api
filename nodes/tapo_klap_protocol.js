...
class KlapProtocol extends TapoProtocol {
        ...
        const response = await this.session_post(url, payload, this._session.get_cookies()[1], { 'seq': seq })
            ...
            if (value[0].status != 200) {
                ...
                const svr_answer = await (new TapoResponse()).try_from_json(JSON.parse(this._session.chiper.decrypt(value[1])))
                    .then((value) => { return value; })
                    .catch((error) => { throw new TapoError(ErrorMsg[ErrorCode[ErrorCode.ERROR_FUNC_GENERAL]] + 'Functional error - ' + error.message, error, ErrorCode.ERROR_FUNC_GENERAL, this._send_request.name); });
                return svr_answer;
            }
        })
            .catch((error) => {
    ...
    async close(change_agent) {
        // Process parameters
        const change = (typeof (change_agent) == 'undefined' ? false : change_agent);
        ...
        if (change) {
            ...
            this._http_agent = (new URL(this._base_url).protocol == 'https:' ? new node_https_1.default.Agent({ keepAlive: this._keep_alive, timeout: AXIOS_TIMEOUT }) : new node_http_1.default.Agent({ keepAlive: this._keep_alive, timeout: AXIOS_TIMEOUT }));
        }
    }
}
...
class KlapChiper extends TapoChiper {
    ...
    encrypt(msg) {
        ...
        if (!(msg instanceof Buffer))
            throw new TapoError(ErrorMsg[ErrorCode[ErrorCode.ERROR_KL_ENCRYPT_FMT]] + "El tipo no es Buffer - " + typeof (msg), null, ErrorCode.ERROR_KL_ENCRYPT_FMT, this.encrypt.name);
    ...
    _iv_seq() {
        ...
        if (iv.length != 16)
            throw new TapoError(ErrorMsg[ErrorCode[ErrorCode.ERROR_KL_ENCRYPT_IV_LENGTH]] + "La longitud es " + iv.length, null, ErrorCode.ERROR_KL_ENCRYPT_IV_LENGTH, this._iv_seq.name);
        return iv;
    }
}
...
class TapoDevice {
    // Constructor to initialize the class
    constructor(terminal_random, api, keep_alive, debug) {
        this._api = api;
    ...
    async cloud_login(auth_credential) {
        ...
        const config = { url: CLOUD_URL, method: 'post', data: request, timeout: AXIOS_TIMEOUT, httpAgent: new node_http_1.default.Agent({ keepAlive: false }), validateStatus: () => { return true; }, proxy: false };
    ...
    async list_devices(cloudToken) {
        ...
        const config = { url: CLOUD_URL + '?token=' + cloudToken, method: 'post', data: request, timeout: AXIOS_TIMEOUT, httpAgent: new node_http_1.default.Agent({ keepAlive: false }), validateStatus: () => { return true; }, proxy: false };
        const response = await axios_1.default.request(config)
            .then(async (value) => {
            if (value.status != 200) {
